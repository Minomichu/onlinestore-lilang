...
* 
* Ändringar gjorda: 
* Körde copy-paste på if/else-satsen för checkout-button och la den även i smallBasketEl,
* annars visades den inte när man la till något utan endast efter att ha uppdaterat sidan. 
* Ändrade till 4 spalter så artikelnumret inte visas
* Utöver det endast mindre ändringar med CSS och texter. 
*/
        ...
        if (basketItems.length > 0) {
            ...
            if (document.getElementsByClassName("checkout-button")) {
                for (var i = 0; i < checkoutButtonEl.length; i++) {
                    ...
                    console.log("Ska visas som block");
                } 
            }
        } else {
            // Tomt i listan
...
function showSmallBasket() {
    if (document.getElementById("small-basket")) {
        ...
        if (basketItems.length > 0) {
            ...
            smallBasketEl.innerHTML = "Totalsumma<br/>" + sum + ":-";

            
            //Hit kopierade jag if/else-satsen
             // Visa "Visa kassan"-knappar
             if (document.getElementsByClassName("checkout-button")) {
                for (var i = 0; i < checkoutButtonEl.length; i++) {
                    checkoutButtonEl[i].style.display = "block";
...
function showCheckout() {
    if (document.getElementById("checkout")) {
        ...
        if (basketItems.length > 0) {
            ...
            // Lägg till summan sist
            checkoutEl.innerHTML += "<tr>" +
                "<td colspan='4' style='color:#F21DA0' class='checkout-sum'>Slutsumma: " + sum + ":-</td>";
...
function checkoutBasket() {
    ...
    if (basketItems != null) {
        ...
        if (itemCount == 1) {
            alert("Inga avbeställningar!\nInga återköp!\nHa betalningen redo (" + totalSum + " kr).");
        } else {
            alert("Inga avbeställningar!\nInga återköp!\nHa betalningen redo (" + totalSum + " kr).");
        }
        ...
        alert("Hur full är du egentligen?\nVälj vad du vill ha innan du försöker beställa..."); 
    }
}
...
function emptyBasket(conf = true) {
    if (conf == true) {
        if (confirm("Kom igen du vill ju dricka! Säkert att du ska radera?")) {
